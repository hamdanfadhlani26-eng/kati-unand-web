...
/* ── JobCard ── */
function JobCard({ job, index, onShare }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "1.25rem 1.4rem", flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        {/* Footer */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "0.6rem", marginTop: "auto", paddingTop: "0.75rem", borderTop: "1px solid rgba(15,23,42,0.06)" }}>
          ...
          <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
            ...
            <div style={{ position: "relative" }}>
              <motion.button
                ...
                title="Bagikan loker ini"
                style={{
                  display: "inline-flex", alignItems: "center", gap: "0.3rem",
                  background: shareOpen ? "#eff6ff" : "#f8fafc",
                  border: `1px solid ${shareOpen ? "#bfdbfe" : "rgba(15,23,42,0.1)"}`,
                  color: shareOpen ? "#2563eb" : "#64748b",
                ...
                {shareOpen && (
                  <motion.div
                    ...
                    style={{
                      ...
                      border: "1px solid rgba(15,23,42,0.08)",
                      padding: "0.6rem", minWidth: "190px", zIndex: 50,
                    }}
                  >
                    <div style={{ fontSize: "0.7rem", fontWeight: 700, color: "#94a3b8", letterSpacing: "0.06em", textTransform: "uppercase", padding: "0.2rem 0.5rem 0.5rem" }}>Bagikan via</div>
                    {/* Copy link */}
                    <button
                      onClick={() => { onShare(job.id, "copy"); setShareOpen(false); }}
                      style={{ display: "flex", alignItems: "center", gap: "0.6rem", width: "100%", padding: "0.5rem 0.6rem", background: "none", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "0.83rem", color: "#0f172a", fontFamily: "var(--font-body), sans-serif", fontWeight: 500, transition: "background 0.15s" }}
                      onMouseEnter={e => e.currentTarget.style.background = "#f1f5f9"}
                      onMouseLeave={e => e.currentTarget.style.background = "none"}
                    >
                      <span style={{ width: "28px", height: "28px", background: "#f1f5f9", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    ...
                    {/* WhatsApp */}
                    <button
                      onClick={() => { onShare(job.id, "whatsapp", job); setShareOpen(false); }}
                      style={{ display: "flex", alignItems: "center", gap: "0.6rem", width: "100%", padding: "0.5rem 0.6rem", background: "none", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "0.83rem", color: "#0f172a", fontFamily: "var(--font-body), sans-serif", fontWeight: 500, transition: "background 0.15s" }}
                      onMouseEnter={e => e.currentTarget.style.background = "#f0fdf4"}
                      onMouseLeave={e => e.currentTarget.style.background = "none"}
                    >
                      <span style={{ width: "28px", height: "28px", background: "#dcfce7", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="#16a34a">
                          ...
                          <path d="M11.99 2h-.02C6.462 2 2 6.462 2 12c0 1.99.581 3.843 1.585 5.401L2.16 21.38a.5.5 0 00.613.613l3.979-1.424A9.95 9.95 0 0012 22c5.523 0 10-4.477 10-10S17.523 2 11.99 2z"/>
                        </svg>
                      </span>
                      WhatsApp
                    </button>
                    {/* Twitter/X */}
                    <button
                      onClick={() => { onShare(job.id, "twitter", job); setShareOpen(false); }}
                      style={{ display: "flex", alignItems: "center", gap: "0.6rem", width: "100%", padding: "0.5rem 0.6rem", background: "none", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "0.83rem", color: "#0f172a", fontFamily: "var(--font-body), sans-serif", fontWeight: 500, transition: "background 0.15s" }}
                    ...
                    {/* Lihat Detail */}
                    <div style={{ borderTop: "1px solid rgba(15,23,42,0.06)", marginTop: "0.4rem", paddingTop: "0.4rem" }}>
                      <a
                        href={`/job-post/${job.id}`}
                        style={{ display: "flex", alignItems: "center", gap: "0.6rem", width: "100%", padding: "0.5rem 0.6rem", background: "none", borderRadius: "8px", textDecoration: "none", fontSize: "0.83rem", color: "#2563eb", fontFamily: "var(--font-body), sans-serif", fontWeight: 600 }}
                        onMouseEnter={e => e.currentTarget.style.background = "#eff6ff"}
                        onMouseLeave={e => e.currentTarget.style.background = "none"}
                      >
                        <span style={{ width: "28px", height: "28px", background: "#eff6ff", borderRadius: "8px", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
...
export default function JobPostPage() {
  ...
  function handleShare(jobId, method, job) {
    const url = `${window.location.origin}/job-post/${jobId}`;
    if (method === "copy") {
      navigator.clipboard.writeText(url).then(() => {
        setShareToast("✅ Link loker berhasil disalin!");
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "3rem 2rem 5rem" }}>
        ...
        {loadingJobs ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "1.25rem" }}>
            {filtered.map((job, i) => <JobCard key={job.id} job={job} index={i} onShare={handleShare} />)}
          </div>
        )}
      </div>
